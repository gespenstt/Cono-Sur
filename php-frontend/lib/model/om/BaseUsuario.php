...
abstract class BaseUsuario extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the usu_ganador field.
	 * Note: this column has a database default value of: 0
	 * @var        int
	 */
	protected $usu_ganador;
	...
	/**
	 * Get the [usu_ganador] column value.
	 * 
	 * @return     int
	 */
	public function getUsuGanador()
	{
		return $this->usu_ganador;
	}
	...
	public function setUsuEstado($v)
	{
		...
		if ($this->usu_estado !== $v || $this->isNew()) {
			$this->usu_estado = $v;
	...
	/**
	 * Set the value of [usu_ganador] column.
	 * 
	 * @param      int $v new value
	 * @return     Usuario The current object (for fluent API support)
	 */
	public function setUsuGanador($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->usu_ganador !== $v || $this->isNew()) {
			$this->usu_ganador = $v;
			$this->modifiedColumns[] = UsuarioPeer::USU_GANADOR;
		}
	...
	{
			if ($this->usu_estado !== 0) {
				return false;
			}

			if ($this->usu_ganador !== 0) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->usu_email = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			$this->usu_estado = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->usu_ganador = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setUsuEmail($arr[$keys[1]]);
		...
		if (array_key_exists($keys[5], $arr)) $this->setUsuGanador($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioPeer::USU_GANADOR)) $criteria->add(UsuarioPeer::USU_GANADOR, $this->usu_ganador);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setUsuEmail($this->usu_email);
		...
		$copyObj->setUsuGanador($this->usu_ganador);
	 ...
	 * related UsuarioRecetas from storage. If this Usuario is new, it will return
		...
		if ($this->collUsuarioRecetas === null) {
			if ($this->isNew()) {
			   $this->collUsuarioRecetas = array();
				...
				$criteria->add(UsuarioRecetaPeer::USU_ID, $this->usu_id);
	 ...
	 */
	public function getUsuarioRecetasJoinReceta($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UsuarioPeer::DATABASE_NAME);
		}
		...
		if ($this->collUsuarioRecetas === null) {
			if ($this->isNew()) {
				...
				$this->collUsuarioRecetas = UsuarioRecetaPeer::doSelectJoinReceta($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
